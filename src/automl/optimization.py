"""
This script provides functions for automating machine learning model selection, hyperparameter optimization, and feature selection using Optuna.
"""
...
from src.automl.study import optimize_hyperparameters
from src.scripts.feature_selection import select_important_features
...
def _find_best_hyperparameters(data, hyperparameters, cv, trials, patience, trial_type, n_jobs, task_name, model_name, metric_name, test_mode):
	"""
	...
	"""
	best_hyperparameters = optimize_hyperparameters(data, cv, hyperparameters, trials, int(np.ceil(patience * trials)), trial_type, n_jobs, task_name, model_name, metric_name, test_mode)
	
	if not best_hyperparameters:
		raise Exception('Hyperparameter optimization did not find any valid hyperparameters. Please check your input data, hyperparameter search space, or "trials" value.')
	
	return best_hyperparameters


def _validate_task_model_metric(task_name, model_name, metric_name):
	"""
...
def find_best_model(task_name, model_name, metric_name, x_data, y_data, weight_data, cv, n_trials_long, n_trials_short, patience=0.1, n_jobs=-1, drop_rate=0.5, min_columns_to_keep=8, test_mode=False):
	"""
	Automates model selection, hyperparameter optimization, and feature selection.
	...
	with warnings.catch_warnings(record=True) as _warnings:
		best_hyperparameters = _find_best_hyperparameters([x_data, y_data, weight_data], {}, cv, n_trials_long, patience, 'long', n_jobs, task_name, model_name, metric_name, test_mode)
	...
	with warnings.catch_warnings(record=True) as _warnings:
		important_features_list = select_important_features([x_data, y_data, weight_data], cv, best_hyperparameters, drop_rate, min_columns_to_keep, task_name, model_name, metric_name)
		warnings_list.extend([(str(warning.category), str(warning.message)) for warning in _warnings])
	...
	with warnings.catch_warnings(record=True) as _warnings:
		best_hyperparameters = _find_best_hyperparameters([x_data[important_features_list], y_data, weight_data], best_hyperparameters, cv, n_trials_short, patience, 'short', n_jobs, task_name, model_name, metric_name, test_mode)
		warnings_list.extend([(str(warning.category), str(warning.message)) for warning in _warnings])
